...
class Normal:
    def __init__(self, mu, log_sigma, temp=1., clamp = False, clamp_val = -3):
        self.mu = mu
        self.mu = 5. * torch.tanh(mu / 5.)
        if clamp:
            log_sigma = torch.clamp(5. * torch.tanh(log_sigma / 5.), min=clamp_val)
        else:
...
class DiscMixLogistic:
    ...
    def sample(self, t=1.):
        gumbel = -torch.log(- torch.log(torch.Tensor(self.logit_probs.size()).uniform_(1e-5, 1. - 1e-5).cuda()))
